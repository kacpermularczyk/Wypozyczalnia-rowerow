...
public class UserRoleDAO {
	...
	public long howManyUsersHaveRole(int idRole) {
		Query query = em.createQuery("SELECT COUNT(ur) FROM UserRole ur WHERE ur.role.idRole = :idRole AND ur.role.isActive = 1 AND ur.isActive = 1");
	...
	public List<String> getUserActiveRolesById(int id) {
		List<String> list = null;
		
		Query query = em.createQuery("SELECT ur.role.role FROM UserRole ur where ur.isActive = 1 AND ur.user.idUser = :id");
	...
	public void removeAllUserRoles(UserRole ur, Date dateNow) {
		Query query = em.createQuery("UPDATE UserRole ur SET ur.isActive = 0, ur.activeUntil = :dateNow WHERE ur.user.idUser = :id AND ur.isActive = 1");
		
		query.setParameter("id", ur.getUser().getIdUser());
