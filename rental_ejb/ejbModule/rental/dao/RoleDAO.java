...
public class RoleDAO {
	...
	public long getAmountOfRolesUserDoesNotHave(int idUser) {
	...
	public Role getRoleIdByRoleName(String role) {
		Query query = em.createQuery("SELECT r FROM Role r WHERE r.isActive = 1 AND r.role = :role");
		
		query.setParameter("role", role);
		
		Role roleToReturn = (Role) query.getSingleResult();
